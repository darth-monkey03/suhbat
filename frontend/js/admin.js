...
document.getElementById('fatwa-title').addEventListener('input', e => {
  const slug = e.target.value
    ...
    .trim()
    .replace(/\s+/g, '-');
  document.getElementById('fatwa-slug').value = slug;
});
...
document.getElementById('fatwa-submit').addEventListener('click', async () => {
  ...
  if (!title || !slug || !content) {
    showMsg(msg, 'error', 'Title, slug, and content are required.');
    return;
  }
  ...
  try {
    const res = await fetch(`${API}/articles`, {
      ...
      body: JSON.stringify({ title, slug, excerpt, content, category_id: 1, author })
    });
    ...
    if (res.ok) {
      ...
      showMsg(msg, 'error', `Error: ${data.error}`);
    }
  } catch (err) {
    showMsg(msg, 'error', 'Could not connect to server. Is it running?');
  }

  btn.disabled = false;
  btn.textContent = 'Publish Fatwa';
});
...
document.getElementById('video-submit').addEventListener('click', async () => {
  ...
  if (!title || !url) {
    showMsg(msg, 'error', 'Title and YouTube URL are required.');
    return;
  }
  ...
  try {
    const res = await fetch(`${API}/videos`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title, youtube_url: url, description: desc, author })
    });
    const data = await res.json();
    if (res.ok) {
...
async function loadList() {
  ...
  try {
    const [artRes, vidRes] = await Promise.all([
      fetch(`${API}/articles?category=fatawa&limit=50`),
      ...
      ? artData.articles.map(a => `
          <div class="list-item">
            <div>
              ...
              <div class="list-item__date">${new Date(a.created_at).toLocaleDateString('en-GB', {day:'numeric',month:'long',year:'numeric'})}</div>
            </div>
